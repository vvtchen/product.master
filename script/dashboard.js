...
async function chart() {
  ...
  const sales = await fetch(`/dbChartSales?year=${year}`);
  const salesData = await sales.json();
  const lastSales = await fetch(`/dbChartSales?year=${year - 1}`);
  const lastSalesData = await lastSales.json();
  console.log(salesData);
  console.log(lastSalesData);
  const invoice = await fetch(`/dbChartInvoice?year=${year}`);
  const invoiceData = await invoice.json();
  const lastInvoice = await fetch(`/dbChartInvoice?year=${year - 1}`);
...
async function getThisYear() {
  ...
  const sales = await fetch(`/salesData?year=${year}`);
  const invoice = await fetch(`/invoiceData?year=${year}`);
  const salesData = await sales.json();
  const invoiceData = await invoice.json();
  ...
  const salesLast = await fetch(`/salesData?year=${year - 1}`);
  const invoiceLast = await fetch(`/invoiceData?year=${year - 1}`);
  ...
  if (salesDataLast[0].SKUCount !== 0 && invoiceDataLast[0].SKUCount !== 0) {
    console.log(salesDataLast);
    console.log(invoiceDataLast);
